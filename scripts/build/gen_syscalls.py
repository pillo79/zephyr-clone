...
api_template = """/* auto-generated by gen_syscalls.py, don't edit */

#ifndef ZEPHYR_API_LIST_H
#define ZEPHYR_API_LIST_H
...
def define_type(argtype):
    toks = re.findall(r"\w+|\W+", argtype)
    toks = [ x.strip() for x in toks ]
    toks = [ x for x in toks if not x in ("", "const") ]

    for known_type in ( 
            "short", "long", "int",     # base types
            "void", "bool", "char",
            "int8_t", "uint8_t",        # from stdint.h
            "int16_t", "uint16_t",
            "int32_t", "uint32_t",
            "int64_t", "uint64_t",
            "intptr_t", "uintptr_t",
        ...
        if known_type in toks:
            return None

    if toks[0] in ("enum", "struct"):
        # emit proper forward declaration
        return "%s %s;" % (toks[0], toks[1])

    return "typedef %s %s;" % ("uint64_t" if need_split(toks[0]) else "uint32_t", toks[0])

def fake_api_def(func_name, func_type, args, fn):
    types = set()

    types.add(define_type(func_type))
    for argtype, argname in args:
        types.add(define_type(argtype))

    decl_arglist = ", ".join([" ".join(argrec) for argrec in args]) or "void"
    decl = "%s %s(%s);" % (func_type, func_name, decl_arglist)
...
def analyze_fn(match_group, fn):
    ...
    types, decl = fake_api_def(func_name, func_type, args, fn)

    # Entry in _k_syscall_table
    table_entry = "[%s] = %s" % (sys_id, handler)

    return (handler, invocation, types, decl, marshaller, sys_id, table_entry)
...
def parse_args():
    ...
    parser.add_argument("-a", "--api-header",
                        help="external API header")
...
def main():
    ...
    for match_group, fn in syscalls:
        handler, inv, api_types, api_decl, mrsh, sys_id, entry = analyze_fn(match_group, fn)

        if fn not in invocations:
            invocations[fn] = []

        invocations[fn].append(inv)
        api_typedefs.update(api_types) # remove dups
        api_decls.append(api_decl)
        ids.append(sys_id)
    ...
    if args.api_header:
        api_typedefs.remove(None)
        print (api_typedefs)
        print (api_decls)

        api_text = "\n".join(sorted(api_typedefs))
        api_text += "\n\n"
        api_text += "\n".join(api_decls)
        with open(args.api_header, "w") as fp:
            fp.write(api_template % api_text)
